...
PRDs 0-4 define the data, adapters, engine, and UI. PRD-5 wires the operational surface: where config lives, how `wrapper-mems` is installed, who an "author" is, what privacy controls exist, and how the tool reports its own health.
...
- Multi-project per repo (one repo = one project; subprojects use separate `.wrapper-mems/`).
...
1. **Project config** (committed to repo): `.wrapper-mems/config.yaml`
2. **User config** (per-machine, never committed): `~/.config/wrapper-mems/config.yaml`
...
providers:
  ...
  claude-mem:
    ...
    project_path_mapping:
      # claude-mem identifies projects by filesystem path; map them to wrapper-mems project name
      "/Users/agustin/proyects/my-app": my-app
      "/Users/juan/work/my-app":         my-app

sync:
  mirror_engram: false         # opt-in (PRD-1 §5.6, PRD-3 §8)
  default_action: full         # full | pull | push (used by `wrapper-mems sync` with no args)
  auto_commit: false           # if true, sync runs `git add .wrapper-mems/ && git commit`
...
```yaml
# ~/.config/wrapper-mems/config.yaml
sync:
...
```
wrapper-mems init [--force] [--providers engram,claude-mem] [--project NAME]
```

Behavior:

1. Refuse if `.wrapper-mems/` already exists, unless `--force`.
...
4. Create:
   - `.wrapper-mems/schema.json` (PRD-0 §9 shape)
   - `.wrapper-mems/config.yaml` (PRD-5 §4.2 shape, with detected values)
   - `.wrapper-mems/memory.jsonl` (empty file with a single comment header line is NOT used — JSONL must be parsable, so file is truly empty)
5. Update `.gitignore`:
   - Add `.wrapper-mems/index.json`
   - Add `.wrapper-mems/.lock`
...
**Honest limitation**: this is reactive (you have to know the session ID to exclude it). A future v1.1 may add `wrapper-mems exclude <last>` to mark the most recent session.
...
- claude-mem session content is summarized BEFORE wrapper-mems sees it. If the summary leaks information, that's a claude-mem concern, not ours.
...
- **`go install github.com/<org>/wrapper-mems@latest`**: for devs with Go.

### 8.2 v1.1+ channels

- **Homebrew tap**: `brew install <org>/tap/wrapper-mems`. Deferred to v1.1 to avoid maintaining a tap before the binary stabilizes.
- **Linux packages** (apt/rpm): deferred. Snap/Flatpak not planned.

### 8.3 Versioning

- Semantic versioning.
- The binary refuses to operate on a `.wrapper-mems/` whose `wrapper_mems_min_version` exceeds itself.
- `wrapper-mems version` prints binary version + canonical schema version compatibility range.

## 9. `wrapper-mems doctor`
...
✓ canonical store        .wrapper-mems/memory.jsonl (1247 lines, valid)
...
⚠ git                    .wrapper-mems/index.json not in .gitignore
✓ lock                   no stale lock

1 warning. Run `wrapper-mems doctor --fix` to apply safe fixes.
```
...
**None. Not now, not later.** Stated explicitly in `--help` and README. No phone home, no anonymous usage stats, no crash reporting that ships content. If we ever add opt-in error reporting, it would require an explicit `telemetry.enabled: true` in user config AND a `wrapper-mems telemetry enable` flow.
...
2. **Config schema migration**: when `schema_version` of config bumps, do we auto-migrate or refuse? Lean: refuse + emit a `wrapper-mems migrate-config` command. Decide before v0.2 ships.
3. **`wrapper-mems exclude` UX for claude-mem session opt-out**: command shape and where the latest session ID comes from (parse `.claude-mem/logs/`? query worker `/api/sessions?limit=1`?). Defer to v1.1 design.
